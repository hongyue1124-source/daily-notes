...
### 6. `[[术语]]` 引用检查会被嵌套数组误报 —— **2026-09-06 已修**

`insert_issue.py` 原先是把 `sections` 整个 `json.dumps` 之后跑
`\[\[([^\]|]+)(?:\|[^\]]+)?\]\]` 正则的，于是结构里的嵌套数组序列化出的字面
`[[` 会被当成术语引用，报「术语表里没有这个词」。

**触发条件比想象中窄**，实测过：

| 形状 | 序列化后 | 旧逻辑 |
|---|---|---|
| 表格**只有一行** `rows:[["9,333"]]` | `[["9,333"]]` | ✗ 误报 `"9,333"` |
| 单行多列 `rows:[["上海-纽约","9,333"]]` | `[["上海-纽约","9,333"]]` | ✗ 误报整串 |
| `ul`/`ol` 的 `items:[["要点"]]` | `[["要点"]]` | ✗ 误报 |
| **多行**表格 `rows:[["a","1"],["b","2"]]` | `…],["…` | ✓ 不触发 |

关键在于 `[[` 和 `]]` 之间不能夹 `]`——只有**单行/单元素**的嵌套数组才凑得出来。
多行表格反而安全，所以这个 bug 平时不常犯，一犯就莫名其妙。

**已改成**：递归遍历到每一个**字符串值**上再跑正则（`_walk_str()` 生成器），
不在整段 JSON 上跑。上面四种形状全部不再误报，合法的 `[[术语]]` 照常抓到。
